...
class YieldRidgeSimple:
    """Ridge-only model -- used for the low-BW subset where n is too small for a tree-based stack."""

    def __init__(self, alpha=5.0):
        self.pipe = Pipeline([("s", StandardScaler()), ("m", Ridge(alpha=alpha))])
        self.feature_cols = None
    ...
    def fit(self, fe):
        ...
        X, y = fe[self.feature_cols].astype(float), fe[TARGET]
        cv = KFold(n_splits=5, shuffle=True, random_state=42)
        oof = cross_val_predict(self.pipe, X, y, cv=cv)
        rmse = float(np.sqrt(np.mean((y - oof) ** 2)))
        r2 = float(1 - np.sum((y - oof) ** 2) / np.sum((y - y.mean()) ** 2))
        self.pipe.fit(X, y)
        return {"rmse": rmse, "r2": r2}
...
if __name__ == "__main__":
    import sys
    bw_cutoff = float(sys.argv[1]) if len(sys.argv) > 1 else None

    print("Loading dairy dataset...")
    df_raw = pd.read_csv("/mnt/user-data/uploads/Dairy_Data.csv")
    df = df_raw[df_raw["2.-Physiological-status-"] == "lactating"].copy()
    df = df.dropna(subset=[TARGET]).reset_index(drop=True)

    if bw_cutoff:
        df = df[df["2.-Body-weight-(kg)"] <= bw_cutoff].reset_index(drop=True)
        print(f"Restricted to BW <= {bw_cutoff} kg -> n={len(df)} rows "
              f"(DMI {df['4.-DM-intake-(kg/day)'].min():.1f}-{df['4.-DM-intake-(kg/day)'].max():.1f}, "
              f"milk {df[TARGET].min():.1f}-{df[TARGET].max():.1f})")

    fe = build_feature_frame(df)

    if bw_cutoff:
        print("Training Ridge-only model (small-n subset, tree stack would overfit)...")
        model = YieldRidgeSimple()
    else:
        print("Training full stacking ensemble (fast-config, not the full grid search)...")
        model = YieldStack()
    metrics = model.fit(fe)
    print(f"  5-fold OOF: RMSE={metrics['rmse']:.3f} kg/day, R2={metrics['r2']:.3f}")

    diet_mode = df["3.-Diet-type"].mode()[0]
    print(f"  Using diet type '{diet_mode}' as the LP-ration proxy (dominant class in training data)")

    MILK_PRICE = 46.0  # INR/kg
    ...
    frontier.to_csv("profit_frontier.csv", index=False)
